...
ggcorr(cor(Pizza[2:8], use = "complete.obs"), hjust = 1, vjust = 1) +
...
plot(pca_pizza, choix = "ind", invisible = "quali")       #individui
plot(pca_pizza, choix = "var")      #variabili
fviz_pca_biplot(pca_pizza)           #entrambi
...
fviz_pca_ind(pca_pizza, geom.ind = "point", pointshape = 21, pointsize = 2, axes = c(1,2),col.ind = "black", fill.ind = Pizza$brand, palette = "jco", addEllipses = TRUE)
...
plot(pca_pizza, cex=0.8, habillage="brand", select="cos2 0.95")                # select="cos2 0.7": Seleziona gli individui il cui contributo alla varianza è maggiore del 70%. Questo è basato sui quadrati dei coseni (cos2).
